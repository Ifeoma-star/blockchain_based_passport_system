;; Digital Passport System - Core Infrastructure
...
(define-read-only (is-valid-passport? (passport-id (string-utf8 32)))
    (match (map-get? Passports { passport-id: passport-id })
        passport (and
            ...
            (> (get expiry-date passport) stacks-block-height)
        )
        false
    )
)
...
(define-public (add-authority
    ...
    (begin
        ...
        (asserts! (is-none (map-get? PassportAuthorities authority-address))
            err-already-exists
        )
        (map-set PassportAuthorities authority-address {
            name: authority-name,
            active: true,
            authorized-since: stacks-block-height,
        })
        (ok true)
    )
)
...
(define-public (remove-authority (authority-address principal))
    (begin
        (asserts! (is-eq tx-sender contract-owner) err-unauthorized)
        (asserts! (is-some (map-get? PassportAuthorities authority-address))
            err-not-found
        )
        (map-set PassportAuthorities authority-address {
            name: (get name
                (unwrap-panic (map-get? PassportAuthorities authority-address))
            ),
            active: false,
            authorized-since: (get authorized-since
                (unwrap-panic (map-get? PassportAuthorities authority-address))
            ),
        })
        (ok true)
...
(define-public (issue-passport
    ...
    (begin
        ...
        (asserts! (is-none (map-get? PassportNumbers passport-id))
            err-already-exists
        )
...
(define-public (revoke-passport (passport-id (string-utf8 32)))
    (begin
        (asserts! (is-authority tx-sender) err-unauthorized)
        (asserts! (is-some (map-get? Passports { passport-id: passport-id }))
            err-not-found
        )
        (let ((passport (unwrap-panic (map-get? Passports { passport-id: passport-id }))))
            (map-set Passports { passport-id: passport-id }
